...
import json
import logging
from gpt_generator import generate_caller_profiles
from call_worker import call_worker

def main():
    logging.basicConfig( level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    voice_agent_prompt = """# Appointment Scheduling Agent Prompt
    ...
    logging.info("Generating caller profiles using GPT")
    caller_profiles = generate_caller_profiles(voice_agent_prompt, num_profiles=10)
    logging.info("Caller profiles generated successfully")

    for profile in caller_profiles:
        logging.info("Profile generated: %s", profile)
    
    threads = []
    results = []  

    logging.info("Initiating outbound calls concurrently...")
    for profile in caller_profiles:
        thread = threading.Thread(target=call_worker, args=(profile, results))
    ...
    logging.info("All calls completed. Aggregated call logs:")
